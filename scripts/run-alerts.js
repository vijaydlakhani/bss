...
async function sendEmailAlerts(signals, recipientEmail) {
    ...
    try {
        ...
        params.append('message', plainTextBody);
        ...
        console.log(`FormSubmit Raw Response: ${resText.substring(0, 250)}`);

        if (resText.includes('true') || response.ok) {
            console.log(`Successfully dispatched email alert to ${recipientEmail}!`);
        } else {
            console.warn(`FormSubmit Response: ${resText}`);
        }
    } catch (err) {
