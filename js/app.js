...
class AppController {
    ...
    populateDropdowns() {
        // Populate Hour
        this.emailSendHour.innerHTML = "";
        for (let i = 1; i <= 12; i++) {
            const val = String(i).padStart(2, '0');
            const opt = document.createElement("option");
            opt.value = val;
            opt.textContent = val;
            this.emailSendHour.appendChild(opt);
        }
        // Populate Minute
        this.emailSendMinute.innerHTML = "";
        for (let i = 0; i < 60; i += 5) {
            const val = String(i).padStart(2, '0');
            const opt = document.createElement("option");
            opt.value = val;
            opt.textContent = val;
            this.emailSendMinute.appendChild(opt);
        }
        // Populate AM/PM
        this.emailSendAmpm.innerHTML = "";
        ["AM", "PM"].forEach(ampm => {
            const opt = document.createElement("option");
            opt.value = ampm;
            opt.textContent = ampm;
            this.emailSendAmpm.appendChild(opt);
        });
    }
    ...
    applySettingsToUI() {
        ...
        const interval = this.emailSettings.interval || "10:00 AM";
        const parts = interval.split(" ");
        const timeOfDay = parts[0] || "10:00";
        const ampm = parts[1] || "AM";
        const [hour, minute] = timeOfDay.split(":");
        
        this.emailSendHour.value = hour || "10";
        this.emailSendMinute.value = minute || "00";
        this.emailSendAmpm.value = ampm;
