    ...
    dispatchAlertEmail(alertSignals, isTest = false, isSecondEmail = false) {
        ...
        if (gatewayMethod === "simulation") {
            ...
            recipients.forEach(async (recipient) => {
                try {
                    this.addLog(`[SERVERLESS] Submitting payload to ${recipient} via FormSubmit AJAX...`);
                    ...
                    if (resData.success === "true" || resData.success === true) {
                        this.addLog(`[SERVERLESS SUCCESS] Email report delivered to ${recipient}!`);
                    } else {
                        this.addLog(`[SERVERLESS WARNING] FormSubmit response for ${recipient}: ${resData.message || 'Check activation email'}`);
                        alert(`FormSubmit Notice: ${resData.message || 'Please check your email inbox to activate FormSubmit.'}`);
                    }
                } catch (error) {
                    this.addLog(`[SERVERLESS ERROR] Failed to submit to ${recipient}: ${error.message}`);
                }
            });
            this.openEmailModal(subject, recipients.join(", "), bodyHtml);
